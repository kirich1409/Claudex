...
class SessionRepositoryTest {
    ...
    @Test
    fun SessionRepository_insert_and_observe_returnsSession() =
        runTest {
            insertProject()
            val session =
                Session(
    ...
    @Test
    fun SessionRepository_updateStatus_changesStatus() =
        runTest {
            insertProject()
            val session =
                Session("s2", "proj1", "S", SessionEnvironment.Local, ClaudeRunOptions(), SessionStatus.ACTIVE, 3000L)
            sessionRepo.insert(session).getOrThrow()
    ...
    @Test
    fun SessionRepository_delete_removesSession() =
        runTest {
            insertProject()
            val session =
                Session("s3", "proj1", "S", SessionEnvironment.Local, ClaudeRunOptions(), SessionStatus.ACTIVE, 4000L)
            sessionRepo.insert(session).getOrThrow()
